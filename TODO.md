...
- [~] Mechanical dispense animation (v2b): tiles already animate via framer-motion's `layoutId` between hands — `state.wall[i]` from the old seed maps to a different position in the new seed, and `Tile`'s `layoutId={`tile-${tileId}`}` traces the path automatically. The `ShuffleOverlay` backdrop now uses a center-only radial gradient (instead of a uniform black tint) so those table-wide motions stay visible behind the swirl. The "real" dispense — engine state-machine pause that gathers every used tile into a center pile first, then disperses — still needs a `dispensing` phase reducer extension; left for a follow-up.
...
- [ ] Camera-based QR scanning for guests — currently obsolete because the host modal no longer generates a QR (auto-rendering one was hiding the "Start hosting" button on landscape mobile, see the LAN entry above). If a "Show QR" toggle is reintroduced for hosts, the scanner can land via `MediaDevices` + `jsqr` (requires HTTPS, so it'd only run inside the installed Capacitor app).
