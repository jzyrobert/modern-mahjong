...
- [ ] Mechanical dispense animation (v2b): between hands, animate every used tile flowing into the center pile and back out into the new walls. Tiles already animate via framer-motion's `layoutId` when they move from wall to hand — extending this to the between-hand transition needs an engine state-machine pause (a `dispensing` phase) so the client gets a window to play the swirl.
