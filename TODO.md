...
- [ ] Mobile bottom sheets: menu / players panels from the design's `app-mobile.jsx`. The 136-tile reference is shipped as the first sheet (📖 button on the match `TopBar`, opens the `Modal` primitive's new `placement="bottom"` mode); menu + players panels still queued.
