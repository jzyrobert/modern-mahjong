...
- [ ] Mobile bottom sheets: menu panel from the design's `app-mobile.jsx`. The 136-tile reference (📖 TopBar button) and the players panel (tap the GameStatusBar pill) are shipped via the `Modal` primitive's `placement="bottom"` mode. The remaining menu sheet would consolidate Leave + Settings + Game log + Tile reference into a single ☰ button to free TopBar slots on narrow phones.
...
- [x] **mDNS host advertisement (Android)** — `LanServer.advertise({ serviceName, port })` registers `_modernmahjong._tcp.` via `NsdManager`, and `LanServer.startDiscovery()` fires `hostFound { name, host, port }` / `hostLost { name }` events as services come and go. Manifest carries the new `CHANGE_WIFI_MULTICAST_STATE` permission. iOS side is stubbed (`NetService` / `NWBrowser` is a follow-up that piggybacks on the iOS HTTP+WS server). Wiring these events into the `HostLanModal` / `JoinLanModal` so the user no longer types a URL is queued separately.
...
- [x] **Auto-link the module into the published bundle** — `expo-lan-server` is now declared as a `file:./modules/expo-lan-server` dependency in `apps/client/package.json`, so every `eas build` (Android dev / preview / production) ships the Kotlin module linked in. The web bundle still falls back to the "needs dev client" path because `requireOptionalNativeModule` returns `null` in that environment. iOS native builds load the Swift skeleton — `isLanServerAvailable()` reads `true` but `start()` throws "not implemented" until Telegraph (or equivalent) is wired up.
