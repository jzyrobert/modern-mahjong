...
- [x] Mobile bottom sheets: 136-tile reference (via the ☰ menu sheet on the match TopBar), menu (☰ button, consolidates Settings / Game log / Tile reference / Leave), and players (tap the GameStatusBar pill — roster + scores + dealer / your-turn indicators). All three ride the `Modal` primitive's `placement="bottom"` mode.
