...
- [ ] **Static-asset HTTP route in the Kotlin module** — serve the Expo Web bundle from the host so a guest on the same Wi-Fi can join via `http://<host>:port/` in any browser without installing the app. Today guests must already have the app or a build-from-`dist/` browser tab.
